...
trait Filesystem {
    async fn mount(&self, options: std::collections::HashMap<&str, zbus::zvariant::Value<'_>>) -> Result<String>;
    
    async fn unmount(&self, options: std::collections::HashMap<&str, zbus::zvariant::Value<'_>>) -> Result<()>;
}
...
impl Client {
    ...
    pub async fn subscribe_device_added(&self) -> Result<std::pin::Pin<Box<dyn futures::Stream<Item = Result<String>> + Send>>> {
        let object_manager = ObjectManagerProxy::new(
            &self.connection,
            "org.freedesktop.UDisks2",
            "/org/freedesktop/UDisks2"
    ...
    pub async fn subscribe_device_removed(&self) -> Result<std::pin::Pin<Box<dyn futures::Stream<Item = Result<String>> + Send>>> {
        let object_manager = ObjectManagerProxy::new(
            &self.connection,
            "org.freedesktop.UDisks2",
            "/org/freedesktop/UDisks2"
        ...
        let stream = async_stream::stream! {
            ...
            while let Some(signal) = stream.next().await {
                if let Ok(args) = signal.args() {
                    ...
                    if args.interfaces.iter().any(|iface| iface == &"org.freedesktop.UDisks2.Block".to_string()) {
                        yield Ok(args.object_path.to_string());
                    }
                }
            }
...
fn get_property_string(props: &std::collections::HashMap<String, OwnedValue>, key: &str) -> Option<String> {
...
fn get_property_u64(props: &std::collections::HashMap<String, OwnedValue>, key: &str) -> Option<u64> {
    props.get(key).and_then(|v| {
...
fn get_property_bool(props: &std::collections::HashMap<String, OwnedValue>, key: &str) -> Option<bool> {
    props.get(key).and_then(|v| {
        v.downcast_ref::<bool>().ok()
    })
}

fn get_property_object_path(props: &std::collections::HashMap<String, OwnedValue>, key: &str) -> Option<String> {
    props.get(key).and_then(|v| {
...
fn get_property_byte_array(props: &std::collections::HashMap<String, OwnedValue>, key: &str) -> Option<String> {
    props.get(key).and_then(|v| {
        if let Ok(arr) = v.downcast_ref::<zbus::zvariant::Array>() {
            let s: String = arr.iter()
